...
from neural_components.attention import SingularityStrengthRecalibration, SpatialSingularityRecalibration, \
    WeightedSpatialSingularityRecalibration
...
def get_ssr_automatic_scales_unet(channels_per_level, r=2, max_scale=3, input_shape=[224, 224, 3], n_classes=2, with_bn=True):
    input = tf.keras.layers.Input(shape=input_shape)
    n_levels = len(channels_per_level) - 1  # hierarchy levels in U-Net
    skips = []  # keep record of the output of each CNN contracting block
    output_channel_dim = 1 if n_classes <= 2 else n_classes
    ...
    for i, n_channel in enumerate(channels_per_level[:-1]):
        x = ContractingLayer(n_filters=n_channel, kernel_size=6-i, padding='same', with_bn=with_bn)(x)
        x = SingularityStrengthRecalibration(r=r, max_scale=max_scale)(x)
        skips.append(x)
        x = tf.keras.layers.MaxPool2D(pool_size=(2, 2))(x)
    # Bottleneck
...
def get_spatial_automatic_scales_ssr_unet(channels_per_level, r=2, max_scale=3, input_shape=[224, 224, 3], n_classes=2, with_bn=True):
    input = tf.keras.layers.Input(shape=input_shape)
    n_levels = len(channels_per_level) - 1  # hierarchy levels in U-Net
    skips = []  # keep record of the output of each CNN contracting block
    output_channel_dim = 1 if n_classes <= 2 else n_classes
    ...
    for i, n_channel in enumerate(channels_per_level[:-1]):
        x = ContractingLayer(n_filters=n_channel, kernel_size=3, padding='same', with_bn=with_bn)(x)
        x = SpatialSingularityRecalibration(r=r, max_scale=6-i)(x)
        skips.append(x)
        x = tf.keras.layers.MaxPool2D(pool_size=(2, 2))(x)
    # Bottleneck
    x = ContractingLayer(n_filters=channels_per_level[-1], kernel_size=3, padding='same', with_bn=with_bn)(x)
...
def get_weighted_spatial_automatic_scales_ssr_unet(channels_per_level, r=2, max_scale=3, input_shape=[224, 224, 3], n_classes=2, with_bn=True):
    input = tf.keras.layers.Input(shape=input_shape)
    n_levels = len(channels_per_level) - 1  # hierarchy levels in U-Net
    skips = []  # keep record of the output of each CNN contracting block
    output_channel_dim = 1 if n_classes <= 2 else n_classes
    ...
    for i, n_channel in enumerate(channels_per_level[:-1]):
        x = ContractingLayer(n_filters=n_channel, kernel_size=3, padding='same', with_bn=with_bn)(x)
        x = WeightedSpatialSingularityRecalibration(r=r, max_scale=6-i)(x)
        skips.append(x)
        x = tf.keras.layers.MaxPool2D(pool_size=(2, 2))(x)
    # Bottleneck
    x = ContractingLayer(n_filters=channels_per_level[-1], kernel_size=3, padding='same', with_bn=with_bn)(x)
