...
from models.ssr_unet import get_ssr_unet, get_weighted_spatial_automatic_scales_ssr_unet, get_spatial_automatic_scales_ssr_unet
...
LOG_DIR = '/home/miguelmartins/Projects/FractalFCN/logs_scale_free/ten-fold/kvasir/'
...
def main():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(dataset_np_x)):
        ...
        fold_data = ExperimentConfigParser(
            name=f'cossim-ssr-unet-{NUM_FOLDS}-folds-{fold}-kvasir-seg_pid{os.getpid()}',
            config_path=CONFIG_FILE,
            log_dir=LOG_DIR)
        model = get_spatial_automatic_scales_ssr_unet(channels_per_level=fold_data.config.model.level_depth,
